...
const allPathsSourceTarget = function(graph, path = [0]) {
  const paths = [];

  for (let next of graph[path[path.length - 1]]) {

    if (next === graph.length - 1) {
      paths.push(path.concat(next));
    } else {

      for (let completedPath of allPathsSourceTarget(graph, path.concat(next))) {
