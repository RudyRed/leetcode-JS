...
// subdomain "leetcode.com" and "com" will also be visited. So they will all be visited 9001 times.
//
// Example 2:
// Input:
...
// once and "wiki.org" 5 times. For the subdomains, we will visit "mail.com" 900 +
// 1 = 901 times, "com" 900 + 50 + 1 = 951 times, and "org" 5 times.
//
// Notes:
//
...
const subdomainVisits = function(cpdomains) {
  ...
  for (let domain of cpdomains.map(x => x.split(' '))) {
    let str = domain[1];
  ...
  return Object.keys(countObj).reduce((a, i) => a.concat(`${countObj[i]} ${i}`) ,[]);
};
